...
def constructBayesNet(gameState: hunters.GameState):
    ...
    spaces = []
    for r in range(X_RANGE):
        for c in range(Y_RANGE):
            spaces.append((r,c))
    ...
    variableDomainsDict[PAC] = spaces
    variableDomainsDict[GHOST0] = spaces
    variableDomainsDict[GHOST1] = spaces
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for v in eliminationOrder:
            currFactors, joinFactor = joinFactorsByVariable(currFactors, v)
            if len(joinFactor.unconditionedVariables()) != 1:
                currFactors.append(eliminate(joinFactor, v))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        count = 0
        randNum = random.uniform(0, self.total())
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        obsProb = busters.getObservationProbability(noisyDistance, manhattanDistance(pacmanPosition, ghostPosition))
        return obsProb
        #raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for p in self.allPositions:
            for newPos, prob in self.getPositionDistribution(gameState, p).items():
                DiscDist[newPos] += prob* self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        ppp = int(self.numParticles / len(self.legalPositions))
        pCount = 0
        for p in self.legalPositions:
            while pCount < ppp:
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        #if particles have zero weight
        if DiscDist.total() == 0:
            self.initializeUniformly(gameState)
            #change to discdist obj
            DiscDist = self.getBeliefDistribution()

        self.particles = []
        pCount = 0
        
        while pCount < self.numParticles:
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if ghostDist == None:
                    ...
                    prob *= self.getObservationProb(ghostDist, gameState.getPacmanPosition(), p[i], self.getJailPosition(i))
            DiscDist[p] += prob
